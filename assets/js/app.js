...
function mountLuffaModal() {
  ...
  const html = `
    <div class="luffa-modal" id="luffa-modal" onclick="if(event.target===this)closeLuffaModal()">
      <div class="luffa-modal-card">
        ...
        <div class="luffa-modal-stats">
          ...
          <div><strong>${PLATFORM_STATS.settlement_accuracy}%</strong><span>结算正确率</span></div>
        </div>
        <div class="luffa-modal-cta">
          <a href="#" class="btn-primary" onclick="trackEvent('luffa_download_store',{store:'ios'})">App Store 下载</a>
          <a href="#" class="btn-secondary" onclick="trackEvent('luffa_download_store',{store:'android'})">Google Play 下载</a>
        </div>
        <div class="luffa-modal-foot">
          <small>Operated within a group holding HK SFC licenses · Play-money + utility token reward</small>
        </div>
      </div>
    </div>
  `;
  ...
  return `
    <header class="site-header">
      <a href="index.html" class="logo">
        ...
        <span class="logo-text">Goal<em>Rush</em></span>
      </a>
      <nav class="nav">
        <a href="index.html" class="${active==='home'?'active':''}">${navItems.home}</a>
        <a href="matches.html" class="${active==='matches'?'active':''}">${navItems.matches}</a>
        <a href="groups.html" class="${active==='groups'?'active':''}">${navItems.groups}</a>
        <a href="leaderboard.html" class="${active==='leaderboard'?'active':''}">${navItems.leaderboard}</a>
        <a href="my-predictions.html" class="${active==='mypred'?'active':''}">${navItems.mypred}</a>
        <a href="faq.html" class="${active==='faq'?'active':''}">${navItems.faq}</a>
      </nav>
      ${hasI18n ? renderLangSwitcher() : ''}
      <button class="btn-cta-mini" onclick="triggerLuffaDownload('header')">${navItems.download}</button>
    </header>
  `;
}
